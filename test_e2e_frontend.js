...
async function runE2ETests() {
  ...
  await test('Frontend queries Salary Structures and Sequential Rules', async () => {
    ...
    validStructureId = res.data.data[0].id;
  ...
  await test('Frontend loads Step 2 Eligible Employees for Payrun', async () => {
    const res = await request({
      hostname: '127.0.0.1',
      port: 5173,
      path: `/api/payruns/eligible-employees?salaryStructureId=${validStructureId}&periodStart=2026-09-01&periodEnd=2026-09-30`,
      method: 'GET',
      headers: { Authorization: `Bearer ${adminToken}` },
    });
    if (res.status !== 200 || !Array.isArray(res.data.data)) {
  ...
  await test('Frontend executes 2-Step Payrun Wizard creation', async () => {
    const res = await request(
      ...
      },
      {
        ...
        salaryStructureId: validStructureId,
