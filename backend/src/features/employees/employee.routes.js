...
router.patch('/profile-change-requests/:requestId/approve', authorize('ADMIN', 'HR_MANAGER'), (req, res, next) => employeeController.approveProfileRequest(req, res, next));
router.patch('/profile-change-requests/:requestId/reject', authorize('ADMIN', 'HR_MANAGER'), (req, res, next) => employeeController.rejectProfileRequest(req, res, next));
